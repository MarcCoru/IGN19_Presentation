...
{\setbeamercolor{background canvas}{bg=tumbluedark}
	...
	\begin{center}
		\begin{columns}
			...
			\hfill 
			Research Agenda: \qquad $\V{?} = f\left({\sat\left({\earth}\right)}\right)$ \hspace{5em}
%			\column{.5\textwidth}
%			\Huge
%			\begin{center}
%
...
\visible<2-4,9>{
	...
	\node[below=of h2o](annoth2o){Drought/Water};
	\draw[annot] (annoth2o) -- (h2o.south);
	
	\node[below=of sugar](annotsugar){Biomass};
...
\node[above right=3em and 2em of arrow, label=right:$\M{x}$](sat) {\includegraphics[width=10mm, angle=130]{images/icons/sat2}};
...
\visible<5-8>{\item[\color{tumblue}Problem:] \textbf{un/self-supervised learning} of a vegetation model \textbf{is difficult}}
\visible<6-8>{\item[\color{tumblue}Solution:] re-framing as \textbf{supervised classification} of crop type labels $\V{y} \in \{y_\text{corn},y_\text{meadow},\dots\}$}
\visible<8>{\item[\color{tumblue}Intuition:] A \textbf{supervised classification model} must \textbf{internalize} a learned \textbf{discriminative model} for the \textbf{vegetation}}
\end{description}
};
...
\begin{columns}
	...
	{\Large
		\only<1>{
			\begin{equation*}
			\V{y} = f_\text{vegetation}(\V{X}_t)
			\end{equation*}
		}
		\only<2>{
			\begin{equation*}
			\V{y} = f_\text{vegetation}(\V{X}_t,\V{X}_{t+1})
			\end{equation*}
		}
		\only<3->{
			\begin{equation*}
			\V{y} = f_\text{vegetation}(\V{X}_t,\V{X}_{t+1},\V{X}_{t+2})
			\end{equation*}
		}
	}
...
\begin{frame}
\frametitle{Method Analogies in Machine Learning}
\centering
...
\visible<3->{
	\node[fit=(nlp)(mt), draw, inner sep=.5em, rounded corners, thick, label=above:{\bfseries \Large Ph.D. Focus}]{};
}
%\draw[-stealth] (cv) -- (0,0);
...
\begin{frame}
\frametitle{Just Semantic Information Extraction from Sequential Data}
